...
gulp.task('remove-extra-css', async function () {
    const hash = await getHash();
    return deleteAsync([`${CSS_DIR}/*.css`, `!${CSS_DIR}/main-${hash}.css`]);
});

gulp.task('process-css', gulp.series('concat-css', 'minify-css', 'rev-css', 'remove-extra-css'));
...
/**
 * Reads the hash generated by 'css-rev' in ./config/rev-manifest.json.
 * 
 * @param {func} callback : callback function to pass the hash to 
 * @returns {string} hash : hash read from ./config/rev-manifest.json
 */
async function getHash (callback) {
    var hash;
    fs.readFile('./config/rev-manifest.json', 'utf-8', (err, jsonString) => {
        if (err) {
            throw err;
        ...
        const filename = JSON.parse(jsonString)['main.css'];

        let reg = /-([\da-z]+)\./;
        hash = filename.match(reg)[1];
    });
